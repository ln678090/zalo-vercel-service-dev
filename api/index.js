...
const PAYMENT_QR_PATH = path.resolve("./payment-qr.png");
...
app.get("/login-qr", (_, res) => {
  ...
  res.send(
    `<!doctype html><html><head><meta charset="utf-8" /><title>Zalo Login QR</title></head><body style="font-family:sans-serif;padding:24px"><h3>Quét QR để đăng nhập Zalo</h3><p>QR chỉ dùng ở lần đầu hoặc khi session DB hết hạn.</p><img src="data:image/png;base64,${latestQrBase64}" style="max-width:320px;border:1px solid #ccc" alt="Zalo login QR" /></body></html>`,
  );
});
    ...
    busboy.on("finish", async () => {
      try {
        ...
        if (!connected)
          return res
            .status(503)
            .json({ error: "Zalo chưa connected listener" });
        ...
        const result = await api.sendMessage(
          {
            msg: fields.caption || "Ảnh gửi từ hệ thống",
            attachments: [tmpPath],
          },
          threadId,
          ThreadType.User,
